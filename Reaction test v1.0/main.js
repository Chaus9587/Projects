...
const main = () => {
    container.removeEventListener('click', main); // Not allow to start the reaction test when already started
    ...
    let randomWaitTime = ((Math.random() * 3) + 1) * 1000;
...
const checkMisclick = () => {
    container.removeEventListener('click', checkMisclick); // Prevent the user to start another round instantly
...
const checkActualClick = () => {
    container.removeEventListener('click', checkActualClick);
    document.getElementById('container').style.backgroundColor = '#234567';
